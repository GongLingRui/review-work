...
- 通过参考链接解析网页正文并转换为Markdown以便展示
- 后端提供解析接口：`ai_entertainment.reference.preview` 与 `aigc.reference.preview`
- 输入参数：`url`（需要解析的网页链接）
- 返回字段：`title`、`site_name`、`content_markdown`、`content_text`、`fetched_at`

示例请求（AI影视参考链接解析）：

```bash
curl -X POST http://localhost:6788/api/v1/ai_entertainment.reference.preview \
  -H 'Content-Type: application/json' \
  -d '{"url": "https://example.com/ai-film-news"}'
```
...
```json
{
  ...
  "preview": {
    "title": "示例标题",
    "url": "https://example.com/ai-film-news",
    "site_name": "example.com",
    "content_markdown": "# 正文标题\n正文内容...",
    "fetched_at": "2025-11-18T09:00:00Z"
  }
}
```

报告中自动追加“参考内容展示”段落：
- 生成AI影视或AIGC每日报告时，系统会在末尾增加“参考内容展示”
- 自动解析部分参考链接正文并以Markdown嵌入，便于快速阅读